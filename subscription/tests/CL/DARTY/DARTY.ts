...
test('SOF-150935 | WEB-DARTY-CRA', { tag: ['@sanity', '@tnr'],}, async ({ page }) => {
...
test('SOF-155831 | WEB-DARTY-CRS', { tag: ['@sanity', '@tnr'],}, async ({ page }) => {
  const payload = "darty.json"
  const data = craDatas.datas[0];
  var mailrandom = "random" + Date.now() + "@sofinco.fr"
  const random = Math.floor(Math.random() * 48) + 1
  const idRecto = "cn-template-recto-generated-" + random + ".png"
  ...
  const token = await api.getTokenCl()
  const link = await api.getUrlCl(token,"","../../datas/CL/darty.json", "web_darty", "crs")
