...
test('WEB-REDOUTE-CRS', async ({ page }) => {
  ...
  const link = await api.getUrlCl(token,"","../../datas/CL/redoute.json", "web_redoute", "crs")
...
/*test('CEASY-REDOUTE-CRA', async ({ page }) => {
  const data = jdd.datas[0];
  const random = Math.floor(Math.random() * 48) + 1
  const idRecto = "cn-template-recto-generated-" + random + ".png"
  const idVerso = "cn-template-verso-generated-" + random + ".png"
  ...
  const tel = "0757592585"
  const token = await api.getTokenCl()
  const link = await api.getUrlCl(token,"","../../datas/CL/redoute.json", "web_redoute", "vac")
...
test('WEB-T2-REDOUTE-CRA', async ({ page, context }) => {
  const data = jdd.datas[0];
  const random = Math.floor(Math.random() * 48) + 1
  const idRecto = "cn-template-recto-generated-" + random + ".png"
  const idVerso = "cn-template-verso-generated-" + random + ".png"
  ...
  const simulationId = await api.getSimulationRedoute("cra_redoute_simulation.json", "web_redoute", token);
  const link = await api.getUrl(token,simulationId,"cra_redoute_t2.json", "web_redoute", "cra_wis")
